...
function get_locally_preferred_printer() {
    const preferred_printer = localStorage.getItem("OdooPreferredZPLPrinter");
    return preferred_printer ? JSON.parse(preferred_printer)[0] : null;
}
...
registry
    ...
    .add("zpl_handler", async function (action, options, env) {
        ...
        if ("report_type" in action && action.report_type === "qweb-zpl") {
            ...
            try {
                const printer_data = await rpc("/web/dataset/call_kw", {
