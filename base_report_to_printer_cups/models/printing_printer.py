...
class PrintingPrinter(models.Model):
    ...
    def _prepare_update_from_cups(self, cups_connection, cups_printer):
        ...
        try:
            ...
            raise exceptions.UserError(
                _("Failed to get PPD for printer %(printer)s: %(error)s")
                % {"printer": printer_system_name, "error": str(e)}
            ) from e
        ppd_path = ppd_info[2]
        if not ppd_path:
            return vals
    ...
    def print_file(self, file_name, report=None, **print_opts):
        ...
        try:
            ...
            raise exceptions.UserError(
                _("Failed to print on printer %(printer)s: %(error)s")
                % {"printer": self.system_name, "error": str(e)}
            ) from e
    ...
    def cancel_all_jobs(self, purge_jobs=False):
        for printer in self:
            ...
            try:
                ...
                raise exceptions.UserError(
                    _("Failed to cancel jobs on printer %(printer)s: %(error)s")
                    % {"printer": printer.system_name, "error": str(e)}
                ) from e
    ...
    def enable(self):
        for printer in self:
            ...
            try:
                ...
                raise exceptions.UserError(
                    _("Failed to enable printer %(printer)s: %(error)s")
                    % {"printer": printer.system_name, "error": str(e)}
                ) from e
    ...
    def disable(self):
        for printer in self:
            ...
            try:
                ...
                raise exceptions.UserError(
                    _("Failed to disable printer %(printer)s: %(error)s")
                    % {"printer": printer.system_name, "error": str(e)}
                ) from e
        self.mapped("server_id").update_printers()
        return True
    ...
    def print_test_page(self):
        for printer in self:
            ...
            try:
                ...
                raise exceptions.UserError(
                    _("Failed to print test page on printer %(printer)s: %(error)s")
                    % {"printer": printer.system_name, "error": str(e)}
                ) from e
        self.mapped("server_id").update_jobs(which="completed")
        return True
