...
class TestReportCups(TestReport):
    ...
    def test_render_qweb_pdf_printable(self):
        with self.assertLogs(
            ...
            result = super().test_render_qweb_pdf_printable()
    ...
    def test_render_qweb_text_printable(self):
        with self.assertLogs(
            ...
            result = super().test_render_qweb_text_printable()
            self.assertGreaterEqual(len(logs.records), 1)
            for record in logs.records:
                self.assertEqual(record.levelno, logging.WARNING)
            return result
    ...
    def test_print_document_not_printable(self):
        with self.assertLogs(
            ...
            result = super().test_print_document_not_printable()
            self.assertGreaterEqual(len(logs.records), 1)
            for record in logs.records:
                self.assertEqual(record.levelno, logging.WARNING)
            return result
    ...
    def test_print_document_printable(self):
        with self.assertLogs(
            ...
            result = super().test_print_document_printable()
            self.assertGreaterEqual(len(logs.records), 1)
            for record in logs.records:
                self.assertEqual(record.levelno, logging.WARNING)
            return result
