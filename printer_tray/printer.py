...
class Printer(orm.Model):
    ...
    def update_printers_status(self, db_name, uid, context=None):
        ...
        db, pool = pooler.get_db_and_pool(db_name)
        cr = db.cursor()
        res = super(Printer, self).update_printers_status(
            db_name, uid, context=context)
        try:
            connection = cups.Connection()
        ...
        for printer in printer_list:
            ...
            if not printer.tray_ids:
                self._update_tray_option(
                    db_name, uid, printer, context=context)
        return res
